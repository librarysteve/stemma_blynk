...
AUTH = '<INSERT API TOKEN HERE>' # Can be found in the app
...
def read_temp(vpin_num=0):
    ...
    print("Temp {}".format(temp))       # Output message format
...
def read_moisture(vpin_num=1):
    ...
    print("Moisture: {}".format(moisture))  # Output message format, moisture range 0-2000
    blynk.virtual_write(vpin_num, moisture) # Send data to blynk server


@timer.register(vpin_num=2, interval=3, run_once=False) # Interval sets the time between sample readings
